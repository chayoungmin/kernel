...
uint32 Read_Eep_Protect(uint32 Addr)
{
	if(Addr > 127)	return -1;
	return eep[Addr];
}
...
//	Prtc_Init(_Read, _Write);
	...
	//g_OsdData.FLD.PROTOCOL = 3;	//Visca
	//g_OsdData.FLD.PROTOCOL = 1;	//Pelco-P
	g_OsdData.FLD.PROTOCOL = 4;	//Ascii

	g_ViscaData.FLD.BRIGHT_STEP = 1;
	g_ViscaData.FLD.GAIN_STEP = 1;

	g_OsdData.FLD.CAM_TITLE_CH1 = 'A';
	g_OsdData.FLD.CAM_TITLE_CH2 = 'B';
	g_OsdData.FLD.CAM_TITLE_CH3 = 'C';
	g_OsdData.FLD.CAM_TITLE_CH4 = 'D';
	g_OsdData.FLD.CAM_TITLE_CH5 = 'E';
	g_OsdData.FLD.CAM_TITLE_CH6 = 'F';
	g_OsdData.FLD.CAM_TITLE_CH7 = 'G';
	g_OsdData.FLD.CAM_TITLE_CH8 = 'H';
	g_OsdData.FLD.CAM_TITLE_CH9 = 'I';
	g_OsdData.FLD.CAM_TITLE_CH10= 'J';
	g_OsdData.FLD.CAM_TITLE_CH11= 'K';
	g_OsdData.FLD.CAM_TITLE_CH12= 'L';
	g_OsdData.FLD.CAM_TITLE_CH13= 'M';
	...
	while(1)
	{
		...
		if(cmd == '1')	// OSD
		{
			g_UserEepData.FLD.OSD_DEBUG_STATE = 0;
			for(i = 0; i < SCRH; i++)
			{
				for(j = 0; j < SCRW; j++)
				{
			...
			while(run)
			{
				...
				if(_kbhit())
				{
					switch(_getch())
					{
						...
						case 'p':	Osd_Update_Flg(1);		break;
						case 'b':	printf("%X\r\n",IF_OSDGet_pCurmap());	break;
						case 224:
							{
								switch(_getch())
								{
			...
			while(run)
			{
				if(_kbhit())
				{
					switch(_getch())
					{
						case 'q':	run = 0;				break;
						case '1':	
							IF_Send_Reply_Packet(VSC_REPLY_ICR,0);
							IF_Send_Reply_Packet(CNBA_REPLY_PRESET_END,0);
							break;
						case '2':
							IF_Send_Reply_Packet(VSC_REPLY_ICR,1);			
							IF_Send_Reply_Packet(CNBA_REPLY_MOTION_DETECT,0);	
							break;
						case '3':	
							IF_Send_Reply_Packet(VSC_REPLY_MD,0);			
							IF_Send_Reply_Packet(CNBA_REPLY_1SHOT_AF_END,0);	
							break;
						case '4':	
							IF_Send_Reply_Packet(VSC_REPLY_MD,1);			
							IF_Send_Reply_Packet(CNBA_REPLY_MENU_OFF,0);		
							break;
						case '5':	
							IF_Send_Reply_Packet(VSC_REPLY_MD,2);
							IF_Send_Reply_Packet(CNBA_REPLY_REFRESH,0);
							break;
						case '6':
							IF_Send_Reply_Packet(VSC_REPLY_MD,4);
							IF_Send_Reply_Packet(CNBA_REPLY_NON_ZTR_MOVE,0);
							break;
						case '7':
							IF_Send_Reply_Packet(VSC_REPLY_FOCUS,0x1321);
							IF_Send_Reply_Packet(CNBA_REPLY_LENS_INIT_END,0);
							break;
						case '8':	IF_Send_Reply_Packet(VSC_REPLY_ZOOM,0x6120);	break;
						case 'a':	IF_Send_Reply_Packet(VSC_REPLY_WD,0);	break;
						case 'b':	IF_Send_Reply_Packet(VSC_REPLY_WD,1);	break;
					}
				}
				i=0;
...
void id_Print(uint16 id)
{
	switch(id)
	{
		...
		case IDX_PWR_ON :					printf("IDX_PWR_ON");                   break; 
		case IDX_ID_CHK :					printf("IDX_ID_CHK");                   break; 
		case IDX_NULL_RTN :					printf("IDX_NULL_RTN");                 break; 
		case IDX_PRESET_END :				printf("IDX_PRESET_END");               break; 
		case IDX__1SHOT_END :				printf("IDX__1SHOT_END");               break; 
		case IDX_ZTW_STEP :					printf("IDX_ZTW_STEP");                 break; 
		...
		case CADX_CHG_MIRROR :				printf("CADX_CHG_MIRROR");      		break;
		case CADX_DN_STATE :            	printf("CADX_DN_STATE");        		break;
		case CADX_CAM_ANGLE :           	printf("CADX_CAM_ANGLE");       		break;
		case CADX_KEY_ACTION :          	printf("CADX_KEY_ACTION");              break;
		case CADX_ACT_LENS_INITIAL :    	printf("CADX_ACT_LENS_INITIAL");        break;
		case CADX_ACT_CAM_RESET :       	printf("CADX_ACT_CAM_RESET");           break;
		case CADX_SEL_ZM_LIMIT :        	printf("CADX_SEL_ZM_LIMIT");            break;
		case CADX_DAT_ZM_LIMIT :        	printf("CADX_DAT_ZM_LIMIT");            break;
		case CADX_SEL_OSD_STATE :       	printf("CADX_SEL_OSD_STATE");           break;
		case CADX_DAT_OSD_STATE :       	printf("CADX_DAT_OSD_STATE");           break;
		case CADX_SEL_CAM_STATE :       	printf("CADX_SEL_CAM_STATE");           break;
		case CADX_DAT_CAM_STATE :       	printf("CADX_DAT_CAM_STATE");           break;
		case CADX_ACT_ONE_PUSH_AF :     	printf("CADX_ACT_ONE_PUSH_AF");         break;
		case CADX_ACT_STEP_TELE :       	printf("CADX_ACT_STEP_TELE");           break;
		case CADX_ACT_STEP_WIDE :       	printf("CADX_ACT_STEP_WIDE");           break;
		case CADX_TOGLE_DN_MODE :       	printf("CADX_TOGLE_DN_MODE");           break;
		case CADX_TOGLE_ID_ON :         	printf("CADX_TOGLE_ID_ON");             break;
		case CADX_SET_AUTOFOCUS :       	printf("CADX_SET_AUTOFOCUS");           break;
		case CADX_SET_MANUALFOCUS :     	printf("CADX_SET_MANUALFOCUS");         break;
		case CADX_DRT_NZTR :            	printf("CADX_DRT_NZTR");                break;
		case CADX_DRT_SZTR :            	printf("CADX_DRT_SZTR");                break;
		case CADX_DRT_CZTR_ZM :            	printf("CADX_DRT_CZTR_ZM");             break;
		case CADX_DRT_CZTR_DZM :            printf("CADX_DRT_CZTR_DZM");            break;
		case CADX_EPP :            			printf("CADX_EPP");		                break;
		case CADX_EPP_ZOOM :            	printf("CADX_EPP_ZOOM");                break;
		case CADX_EPP_FOCUS :           	printf("CADX_EPP_FOCUS");               break;
		case CADX_EPP_DZOOM :           	printf("CADX_EPP_DZOOM");               break;
		case CADX_EPP_NZTR :            	printf("CADX_EPP_NZTR");                break;
		case CADX_EPP_SZTR :            	printf("CADX_EPP_SZTR");                break;
		case CADX_EPP_QZTR :            	printf("CADX_EPP_QZTR");                break;
		case CADX_IPP_CURRENT :         	printf("CADX_IPP_CURRENT");             break;
		case CADX_IPP_NZTR :            	printf("CADX_IPP_NZTR");                break;
		case CADX_IPP_SZTR :            	printf("CADX_IPP_SZTR");                break;
		case CADX_IPP_QZTR :		    	printf("CADX_IPP_QZTR");                break;

		default :							printf("NO ID");						break; 
	}
}
...
void key_print(uint08 key)
{
	switch(key)
	{
		case KC_NKEY :          printf("KC_NKEY");      	break;
		case KC_TELE :          printf("KC_TELE");      	break;
		case KC_QTELE :         printf("KC_QTELE");     	break;
		case KC_WIDE :          printf("KC_WIDE");      	break;
		case KC_QWIDE :         printf("KC_QWIDE");     	break;
		case KC_FAR :           printf("KC_FAR");       	break;
		case KC_NEAR :          printf("KC_NEAR");      	break;
		case KC_SFAR :          printf("KC_SFAR");      	break;
		case KC_SNEAR :         printf("KC_SNEAR");     	break;
		case KC_STOP :          printf("KC_STOP");      	break;
		case KC_POWER :         printf("KC_POWER");     	break;
		case KC_FOCUS_AM :      printf("KC_FOCUS_AM");  	break;
		case KC_F_PUSH :        printf("KC_F_PUSH");    	break;
		case KC_M_SET :         printf("KC_M_SET");     	break;
		case KC_INITIAL :       printf("KC_INITIAL");   	break;
		case KC_OSD_ONOFF :     printf("KC_OSD_ONOFF"); 	break;
		case KC_MENU :          printf("KC_MENU");      	break;
		case KC_SET_MENU :      printf("KC_SET_MENU");  	break;
		case KC_LANGUAGE :      printf("KC_LANGUAGE");  	break;
		case KC_BL :            printf("KC_BL");        	break;
		case KC_FLICKER :       printf("KC_FLICKER");   	break;
		case KC_SSC_UP :        printf("KC_SSC_UP");    	break;
		case KC_SSC_DN :        printf("KC_SSC_DN");    	break;
		case KC_D_EFT :         printf("KC_D_EFT");     	break;
		case KC_MIRROR :        printf("KC_MIRROR");    	break;
		case KC_NEGA :          printf("KC_NEGA");      	break;
		case KC_MONO :          printf("KC_MONO");      	break;
		case KC_SRP_UP :        printf("KC_SRP_UP");    	break;
		case KC_SRP_DN :        printf("KC_SRP_DN");    	break;
		case KC_BRT_UP :        printf("KC_BRT_UP");    	break;
		case KC_BRT_DN :        printf("KC_BRT_DN");    	break;
		case KC_ZM_SUP :        printf("KC_ZM_SUP");    	break;
		case KC_ZM_SDN :        printf("KC_ZM_SDN");    	break;
		case KC_ZM_EUP :        printf("KC_ZM_EUP");    	break;
		case KC_ZM_EDN :        printf("KC_ZM_EDN");    	break;
		case KC_WB_PUSH :       printf("KC_WB_PUSH");   	break;
		case KC_AWB_UP :        printf("KC_AWB_UP");    	break;
		case KC_AWB_DN :        printf("KC_AWB_DN");    	break;
		case KC_R_UP :          printf("KC_R_UP");      	break;
		case KC_R_DN :          printf("KC_R_DN");      	break;
		case KC_B_UP :          printf("KC_B_UP");      	break;
		case KC_B_DN :          printf("KC_B_DN");      	break;
		case KC_AE_MAN :        printf("KC_AE_MAN");    	break;
		case KC_IRIS_UP :       printf("KC_IRIS_UP");   	break;
		case KC_IRIS_DN :       printf("KC_IRIS_DN");   	break;
		case KC_AGC_UP :        printf("KC_AGC_UP");    	break;
		case KC_AGC_DN :        printf("KC_AGC_DN");    	break;
		case KC_LED :           printf("KC_LED");       	break;
		case KC_AUTOZOOM :      printf("KC_AUTOZOOM");  	break;
		case KC_AGC_OFF :       printf("KC_AGC_OFF");   	break;
		case KC_CINEMA :        printf("KC_CINEMA");    	break;
		case KC_MOSAIC :        printf("KC_MOSAIC");    	break;
		case KC_ART :           printf("KC_ART");       	break;
		case KC_AUTO_PAN :      printf("KC_AUTO_PAN");  	break;
		case KC_PAN_RIGHT :     printf("KC_PAN_RIGHT"); 	break;
		case KC_PAN_LEFT :      printf("KC_PAN_LEFT");  	break;
		case KC_TILT_UP :       printf("KC_TILT_UP");   	break;
		case KC_TILT_DN :       printf("KC_TILT_DN");   	break;
		case KC_AUTO_TILT :     printf("KC_AUTO_TILT"); 	break;
		case KC_STELE :         printf("KC_STELE");     	break;
		case KC_SWIDE :         printf("KC_SWIDE");     	break;
		case KC_MB_RESET :      printf("KC_MB_RESET");  	break;
		case KC_FREEZE :        printf("KC_FREEZE");    	break;
		case KC_REVERSE :       printf("KC_REVERSE");   	break;
		case KC_1SHOT_AF :      printf("KC_1SHOT_AF");  	break;
		case KC_DAYNIGHT :      printf("KC_DAYNIGHT");  	break;
		case KC_MAXFLDUP :      printf("KC_MAXFLDUP");  	break;
		case KC_MAXFLDDN :      printf("KC_MAXFLDDN");  	break;
		case KC_DBGMENU :       printf("KC_DBGMENU");   	break;
		case KC_TECHMENU :      printf("KC_TECHMENU");  	break;
		case KC_OUT_FMT :		printf("KC_OUT_FMT");		break;
		default :				printf("NO_KEY");			break;
	}
}