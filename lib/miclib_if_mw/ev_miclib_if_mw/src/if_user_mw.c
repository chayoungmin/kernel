...
typedef void (*setfnc)(uint32 , uint32);
typedef uint32 (*getfnc)(uint32 );
/*************************************************************************************************************
* LOCAL DEFINES
*************************************************************************************************************/
typedef enum   {
...
/* DAY/NIGHT */
const OSD_ITEM OSD_DBDlyItem[] = {	{OSD_STR_1SEC, 0},	{OSD_STR_2SEC, 0},	{OSD_STR_3SEC, 0},	{OSD_STR_4SEC, 0},	{OSD_STR_5SEC, 0},
									{OSD_STR_6SEC, 0},	{OSD_STR_7SEC, 0},	{OSD_STR_8SEC, 0},	{OSD_STR_9SEC, 0},	{OSD_STR_10SEC, 0},
									{OSD_STR_FINISH, 0}	};
...
*	리턴되는 결과값은 위와 같다.
*************************************************************************************************************/
static IFER CnbAsc_Evt_Proc_Act(IFEVT_PARAM *param)
{

	IFER err = IF_ERNO;
	switch(param->eepidx)
	{
		case CADX_KEY_ACTION:
#ifdef WIN32
#endif
		break;
		default :
...
void IFU_OSD_User_Init()
{
	...
	/***************************************************************/
	// If_mw CNB HEX Technical Protocol initialize
	/***************************************************************/
	IF_CNBT_Regist_Callbeck(CNBT_EEP		,EepRead			,EepWrite			);
	IF_CNBT_Regist_Callbeck(CNBT_ISP		,IspReg_Get			,IspReg_Set			);
	IF_CNBT_Regist_Callbeck(CNBT_MOTOR_IC	,(getfnc)MotDrvRegRead	,(setfnc)MotDrvRegWrite		);
	IF_CNBT_Regist_Callbeck(CNBT_SENSOR_IC	,(getfnc)GetSensSpi		,(setfnc)SetSensSpi			);
	//IF_CNBT_Regist_Callbeck(CNBT_VIDEO_IC		,AD7393_Reg_R		,AD7393_Reg_W		);
	//IF_CNBT_Regist_Callbeck(CNBT_VIDEO_IC2	,MDINI2C_HostReg_R	,MDINI2C_HostReg_W	);
	//IF_CNBT_Regist_Callbeck(CNBT_VIDEO_IC3	,MDINI2C_LocalReg_R	,MDINI2C_LocalReg_W	);
	IF_CNBT_Regist_Callbeck(CNBT_EEP_SKIP	,EepRead			,EepWriteSkip		);
...
IFER IFU_Event_Callback(IFEVT evt, IFEVT_PARAM *param)
{
	...
	/***************************************************************/
	// CNBA Set/Action Event Processing
	/***************************************************************/
	else if(evt == IFEVT_CNBA_SET)
	{
		err = IF_ERNOPROC;
		err = CnbAsc_Evt_Proc_Act(param);
	}
	else if(evt == IFEVT_CNBA_GET)
	{
		err = IF_ERNOPROC;
	}
	/***************************************************************/
