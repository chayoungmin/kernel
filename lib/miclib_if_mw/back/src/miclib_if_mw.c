...
#define OSD_TITLE_LEFT			16 - 3

#define OSD_CURSOR_TOP			(OSD_TITLE_TOP + 2)
#define OSD_MENU_TOP			(OSD_CURSOR_TOP)
#define OSD_ITEM_TOP			(OSD_CURSOR_TOP)

#define OSD_MENU_LEFT			(2)
#define OSD_CURSOR_LEFT			(OSD_MENU_LEFT - 2)
...
typedef struct _PTC_CNBT_CB_FUNC	//Callback Functions
...
typedef struct _IF_VSC_SOCKET{
	uint08 execute;
	PTC_VISCA_CODEC *pCodec;
	uint32 dat[10];
}IF_VSC_SOCKET;
...
static void Visca_Socket_Check();
static uint08 (*Cam_Link_Focus_Drt)(void)	= Cam_Link_Null;
static uint08 (*Cam_Link_Zoom_Drt)(void)	= Cam_Link_Null;
static uint08 (*Cam_Link_ZmFcs_Drt)(void)	= Cam_Link_Null;
static uint08 (*Cam_Link_Reset)(void)		= Cam_Link_Null;
static uint08 (*Cam_Link_set)(void)			= Cam_Link_Null;
static uint08 (*Cam_Link_Recall)(void)		= Cam_Link_Null;

IF_VSC_SOCKET Vsc_socket[2];
/*************************************************************************************************************
* LOCAL TABLES
*************************************************************************************************************/
...
const PLURAL_EVT PluralEvt_ZmFcs[] =
{
	{VDX_ZOOM_POS			,4	,COMPOSE_D_HALF	,DECODE_DIRECT		,&Drt_dat_ZoomPos		},
	{VDX_FOCUS_POS			,4	,COMPOSE_D_HALF	,DECODE_DIRECT		,&Drt_dat_NearLmt		},
	{IDX_NULL				,0	,0				,0					,0						}
};
...
PTC_VISCA_CODEC gPtcVisca_Cmd_ZmFcs =
	{	0x47	,VDX_ZM_FCS_POS			,8 	,DECODE_PLURAL		,&PluralEvt_ZmFcs		};
const PTC_VISCA_CODEC gPtcVisca_Cmd[]= {
	...
	{	0x3F	,VDX_CAM_MEMCUS			,2 	,DECODE_PLURAL		,PluralEvt_CS_Preset	},
...
IF_CONDITION Cndtn_Wb_Push[]	= {
									...
									{VDX_WB_MODE	,	6}, {VDX_WB_MODE	,	7},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL		,	0}
								  };
...
IF_CONDITION Cndtn_Shutter[]	= {
									...
									{VDX_DEFOG_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}
								  };
...
IF_CONDITION Cndtn_Lens[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_ZoomDrt[]	= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_FcsDrt[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_F_MODE		,	0},	{IDX_NULL		,	0} };
IF_CONDITION Cndtn_ZmFcsDrt[]	= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZOOM_COMMAND,	1}, {VDX_FOCUS_COMMAND,	1}, {VDX_F_MODE		,	0},	{IDX_NULL		,	0} };
IF_CONDITION Cndtn_ZmMove[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_ZOOM_COMMAND,	1},	{IDX_NULL		,	0} };
IF_CONDITION Cndtn_FcsMan[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_FOCUS_COMMAND,	1},	{VDX_F_MODE		,	0},	{IDX_NULL		,	0} };
IF_CONDITION Cndtn_Fcs[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_FOCUS_COMMAND,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_MemRecall[]	= {	{VDX_ZMFCS_COMMAND,	1}, {VDX_ZOOM_COMMAND,	1},	{VDX_FOCUS_COMMAND,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_MemReSet[]	= {	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_ZOOM_COMMAND,	1},	{VDX_FOCUS_COMMAND,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_LensInit[]	= {	{VDX_CAMERA_RECALL,	1}, {VDX_ZMFCS_COMMAND,	1}, {VDX_ZOOM_COMMAND,	1},	{VDX_FOCUS_COMMAND,	1},	{IDX_NULL		,	0}};
IF_CONDITION Cndtn_DZoom[]		= {	{VDX_FREEZE_ON	,	1},	{VDX_CAMERA_RECALL,	1}, {VDX_DZOOM_MODE,	0},	{IDX_NULL		,	0}	};
...
#endif
IF_CONDITION Cndtn_AE_Auto[]	= {{VDX_AE_MODE		,	1}, {VDX_AE_MODE	,	2}, {VDX_AE_MODE	,	3}, {VDX_AE_MODE	,	4}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL	,	0}};
IF_CONDITION Cndtn_Freeze[]		= {{VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Freeze_off[]	= {{VDX_FREEZE_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};

IF_CONDITION Cndtn_Ext_ExComp[]		= {{VDX_EX_EXPC_ON		,	0},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Ext_Icr_F2O[]	= {{VDX_EX_AICR_F2O_ON	,	0},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Ext_Aperture[]	= {{VDX_EX_EXPAP_ON		,	0},	{VDX_FREEZE_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Ext_Cr_CGCH[]	= {{VDX_EX_CGCH_ON		,	0},	{VDX_FREEZE_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Nrm_ExComp[]		= {{VDX_EX_EXPC_ON		,	1},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Nrm_Icr_Thres[]	= {{VDX_EX_AICR_F2O_ON	,	1},	{VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Nrm_Aperture[]	= {{VDX_EX_EXPAP_ON		,	1},	{VDX_FREEZE_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
IF_CONDITION Cndtn_Nrm_CGCH[]		= {{VDX_EX_CGCH_ON		,	1},	{VDX_FREEZE_ON	,	1}, {VDX_CAMERA_RECALL,	1},	{IDX_NULL ,	0}};
/************************************* Structure for VISCA Condition *****************************************/
IF_CONDITIONS IFU_Cndtn[MAX_CONDITION] =
{
	{IDX_NULL 				,IDX_NULL				},//!!!!!반드시 포함되야 됨
	{VDX_ZOOM_POS 			,Cndtn_ZoomDrt			},
	{VDX_DZOOM_MODE			,Cndtn_ZmMove			},
	{VDX_DZOOM_POS 			,Cndtn_DZoom			},
	{VDX_FOCUS_POS 			,Cndtn_FcsDrt			},
	{VDX_F_MODE				,Cndtn_Fcs				},
	{VDX_NEAR_LIMIT			,Cndtn_Fcs				},
	{VDX_AF_SENS			,Cndtn_Lens				},
	{VDX_AF_MODE			,Cndtn_Lens				},
	{VDX_AF_MOVE_TIME		,Cndtn_Lens				},
	{VDX_AF_INTERVAL_TIME	,Cndtn_Lens				},
	...
	{VDX_CE_THRS_VAL		,Cndtn_Freeze_off		},
	{VDX_CE_Y_HI			,Cndtn_Freeze_off		},
	{VDX_CE_CR_HI			,Cndtn_Freeze_off		},
	{VDX_CE_CB_HI			,Cndtn_Freeze_off		},
	{VDX_CE_Y_LI			,Cndtn_Freeze_off		},
	{VDX_CE_CR_LI			,Cndtn_Freeze_off		},
	{VDX_CE_CB_LI			,Cndtn_Freeze_off		},
	...
	{VDX_WD_ALARM	 		,Cndtn_Freeze_off		},
	{VDX_MOVE_RGAIN 		,Cndtn_Wb_Gain			},
	{VDX_MOVE_BGAIN 		,Cndtn_Wb_Gain			},
	{VDX_MOVE_DZOOM			,Cndtn_DZoom			},//On/Off 일때는 Cndtn_ZmMove를 따름.
	{VDX_MOVE_ZOOM	 		,Cndtn_ZmMove			},
	{VDX_MOVE_FOCUS 		,Cndtn_FcsMan			},
	...
	{VDX_CAM_MEMCUS			,Cndtn_MemRecall		},//Set,Reset은 MemSet을 따른다.
	{VDX_FCS_PUSH 			,Cndtn_FcsMan			},//Focus Infinity는 Cndtn_Fcs를 따름.
	{VDX_ZM_FCS_POS 		,Cndtn_ZmFcsDrt			},
	{0xffff 				,IDX_NULL				}//!!!!!반드시 포함되야 됨
};

typedef enum _IF_VSC_SOCKET_CMD
{
	IF_VSC_DELAY_CMD_ZOOM,
	IF_VSC_DELAY_CMD_FOCUS,
	IF_VSC_DELAY_CMD_ZMFCS,
	IF_VSC_DELAY_CMD_RESET,
	IF_VSC_DELAY_CMD_SET,
	IF_VSC_DELAY_CMD_RECALL,
	IF_VSC_DELAY_CMD_ETC,
	IF_VSC_DELAY_CMD_MAX
}IF_VSC_SOCKET_CMD;

typedef struct _IF_SOCKET
{
	IF_VSC_SOCKET_CMD delay_cmd;
	uint08 dat[12];
	PTC_VISCA_CODEC *vsc_codec;
}IF_SOCKET;
...
int32 IF_EEPPrivacyUpdate(IFEVT evt, uint08 zone, EEP_IDX idx, int32 val)
{
	...
	else
	{
		...
		switch(idx)
		{
			case IDX_PRI_STATE:		g_OsdData.Data[priAddr] = (uint08)(g_OsdData.Data[priAddr] & 0xfe) | (val & 0x1);			break;
			case IDX_PRI_TRANS:		g_OsdData.Data[priAddr] = (uint08)(g_OsdData.Data[priAddr] & 0xf9) | ((val & 0x4) << 1);	break;
			case IDX_PRI_COLOR:		g_OsdData.Data[priAddr] = (uint08)(g_OsdData.Data[priAddr] & 0xfe) | ((val & 0xc7) << 3);	break;
			case IDX_PRI_MOSAIC:	g_OsdData.Data[priAddr] = (uint08)(g_OsdData.Data[priAddr] & 0xfe) | ((val & 0xc0) << 6);	break;
			case IDX_PRI_X:			g_OsdData.Data[priAddr + 1] = (uint08)val;						break;
			case IDX_PRI_Y:			g_OsdData.Data[priAddr + 2] = (uint08)val;						break;
			case IDX_PRI_W:			g_OsdData.Data[priAddr + 3] = (uint08)val;						break;
			case IDX_PRI_H:			g_OsdData.Data[priAddr + 4] = (uint08)val;						break;
			case IDX_PRI_P:
...
void PrintItem(uint08 y, const OSD_MAP *pMap)
{
	...
	else if(pMap->pitem != OSD_NULL)
	{
		...
		if(val != -1)
		{
			pName = OSD_ENG[pMap->pitem[val].name][g_OsdData.FLD.OSD_LANGUAGE];
	...
	else
	{
		...
		else
		{
			...
			switch(pMap->eep)
			{
				case IDX_RETURN:	pName = OSD_ENG[gReturnItem[RETURN_VAL]][g_OsdData.FLD.OSD_LANGUAGE];	break;
				case IDX_EXIT:		pName = OSD_ENG[gExitItem[EXIT_VAL]][g_OsdData.FLD.OSD_LANGUAGE];		break;
				case IDX_FRESET:	pName = OSD_ENG[gFResetItem[FRESET_VAL]][g_OsdData.FLD.OSD_LANGUAGE];	break;
				case IDX_REBOOT:	pName = OSD_ENG[gRebootItem[REBOOT_VAL]][g_OsdData.FLD.OSD_LANGUAGE];	break;
				case IDX_INITIAL:
				case IDX_TITLE:
					{
						PrintCall(OSD_ITEM_LEFT + i, OSD_ITEM_TOP + y, OSD_FONTENTER);
			...
			if(pName != OSD_NULL)
			{
				...
				if(pName != OSD_ENG[OSD_STR_NO][g_OsdData.FLD.OSD_LANGUAGE])
				PrintCall(OSD_ITEM_LEFT + i, OSD_ITEM_TOP + y, OSD_FONTENTER);
				else
...
static int32 User_Codition_Update(EEP_IDX idx)
{
	...
	switch(idx)
	{
		case VDX_DZOOM_MODE :			ret	= (g_ViscaData.FLD.DZOOM_MODE);    		break;
		...
		case VDX_FOCUS_COMMAND :		ret	= Cam_Link_Focus_Drt();					break;
		case VDX_ZOOM_COMMAND :			ret	= Cam_Link_Zoom_Drt();					break;
		case VDX_ZMFCS_COMMAND :		ret	= Cam_Link_ZmFcs_Drt();					break;
		case VDX_CAMERA_SET :			ret	= Cam_Link_set();						break;
		case VDX_CAMERA_RESET :			ret	= Cam_Link_Reset();						break;
		case VDX_CAMERA_RECALL :		ret	= Cam_Link_Recall();					break;
...
static IFER Visca_Condition_Check(uint16 vdx,uint32 data)
{
	...
	switch(vdx)
	{
		case VDX_MOVE_DZOOM:
			if(data == 2 || data == 3)
			{
				condition_chk = (IF_CONDITION *)Cndtn_ZmMove;
			}
			break;
		case VDX_INIT_CAM :
			if(data == 1)
			{
				condition_chk = (IF_CONDITION *)Cndtn_LensInit;
				tmp = 0;
			}
		case VDX_CAM_MEMCUS :
			if(data == 0 || data == 1)
			{
				condition_chk = (IF_CONDITION *)Cndtn_MemReSet;
				tmp = 0;
			}
			break;
		case VDX_FCS_PUSH :
			if(data == 2)
			{
				condition_chk = (IF_CONDITION *)Cndtn_Fcs;
				tmp = 0;
			}
			break;
	}
...
IFER IF_Ptc_Vsc_SetDatas(PLURAL_EVT *plu_evt,uint32 *data)
{
	IFER er = IF_ERNO;
	IFER evt_result = IF_ERNO;

	uint08 cnt;

	for(cnt = 0; plu_evt[cnt].eepidx != IDX_NULL ;cnt++)
	{
		evt_result = SendEvent(IFEVT_VISCA_CMD, plu_evt[cnt].eepidx,&data[cnt]);

		switch(evt_result)
		{
			case IF_ERNOPROC :	 // 처리하지 않음
				er |= Processong_CMD(plu_evt[cnt].decode_type ,plu_evt[cnt].eepidx ,data[cnt] ,(void *)plu_evt[cnt].evts);
			break;
...
IFER IF_Ptc_Vsc_SetData(PTC_VISCA_CODEC *pCodec,uint32 data)
{
	IFER er = IF_ERNO;
...
void Visca_Socket_Clear(uint08 num)
{
	if(num == 0 || num == 1)
	{
		Vsc_socket[num].pCodec = 0;
	}
}

uint08 Visca_Socket_State(uint08 num)
{
	uint08 flg = 0;
	uint08 tmp;
	if(Vsc_socket[num].pCodec)
	{
		switch(Vsc_socket[num].pCodec->eepidx)
		{
			case 0 :	//empty
			break;
			case VDX_ZOOM_POS :		if(!Cam_Link_Zoom_Drt())	flg = 1;		break;
			case VDX_FOCUS_POS :	if(!Cam_Link_Focus_Drt())	flg = 1;		break;
			case VDX_ZM_FCS_POS :	if(!Cam_Link_ZmFcs_Drt())	flg = 1;		break;
			case VDX_CAM_MEMCUS :
				tmp = Vsc_socket[num].dat[0]>>8;
				switch(tmp)
				{
					case 0x00 :	if(!Cam_Link_Reset())		flg = 1;		break;
					case 0x01 :	if(!Cam_Link_set())			flg = 1;		break;
					case 0x02 :	if(!Cam_Link_Recall())		flg = 1;		break;
				}
			break;
			default :
				flg = 1;
...
static void Visca_Socket_Check()
{
	uint08 crr = (gPtc_Visca_Socket - 1) & 0x01;
	uint08 nxt = (gPtc_Visca_Socket) & 0x01;
	uint08 cmpt = 0;
	uint08 bmpt = 0;
	uint08 after_act = 0;

	cmpt = Visca_Socket_State(crr);

	if(Vsc_socket[crr].pCodec)
	{
		if(Vsc_socket[crr].pCodec->eepidx)
		{
			if((Vsc_socket[crr].dat[0] >> 8)<2)
			{
				after_act = 1;
			}
		}
	...
	if(after_act)
	{
		if(cmpt)
		{
			if(!Vsc_socket[nxt].execute)
			{
				if(Vsc_socket[crr].pCodec->decode_type == DECODE_PLURAL)
				{
					IF_Ptc_Vsc_SetDatas((PLURAL_EVT *)Vsc_socket[crr].pCodec->evts,Vsc_socket[crr].dat);
				}
				else
				{
					IF_Ptc_Vsc_SetData((PTC_VISCA_CODEC *)Vsc_socket[crr].pCodec,Vsc_socket[crr].dat[0]);
				}
			}
			bmpt = Visca_Socket_State(nxt);
		}
	}
	else
	{
	...
	{
		printf("Cmpt %d \n",cmpt);
		gPtc_Visca_Socket = crr + 1;
		PtcVisca_SendPacket(VISCA_MSG_COMPETE_CMD,0,0);
		Visca_Socket_Clear(crr);
		gPtc_Visca_Socket = nxt +1;
	}
	...
	if(bmpt)
	{
		...
		gPtc_Visca_Socket = nxt + 1;
		PtcVisca_SendPacket(VISCA_MSG_COMPETE_CMD,0,0);
		Visca_Socket_Clear(nxt);
	}
}

uint08 gExecute;
static IFER Visca_Socket_Set(EEP_IDX vdx,uint32 data)
{	//Ack를 이안에서 관리해주자
	IFER er = IF_ERNO;
	uint32 tmp;
	uint32 tmp1;
	...
	if(Vsc_socket[0].pCodec && Vsc_socket[1].pCodec )		//Socket Full
	{
		er = IF_SOCKET_FULL;
	}
	else if(Vsc_socket[0].pCodec || Vsc_socket[1].pCodec)	//one of Socket
	{
		er = IF_PASS;
		tmp = data>>8;

		if(Vsc_socket[0].pCodec)
		{
			bfr = 0;
			crr = 1;
		}
		else if(Vsc_socket[1].pCodec)
		{
			bfr = 1;
			crr = 0;
		}
		tmp1 = Vsc_socket[bfr].dat[0]>>8;
		gPtc_Visca_Socket = crr+1;

		switch(vdx)
		{
			case VDX_ZOOM_POS:
				switch(Vsc_socket[bfr].pCodec->eepidx)
				{
					case VDX_ZOOM_POS:		clr = 1;			break;
					case VDX_FOCUS_POS:		break;
					case VDX_ZM_FCS_POS:	er = IF_NOEXCUTE;	break;
					case VDX_CAM_MEMCUS:
						if(tmp == 0x02)
							er = IF_NOEXCUTE;
						else
			...
			case VDX_FOCUS_POS :
				switch(Vsc_socket[bfr].pCodec->eepidx)
				{
					case VDX_ZOOM_POS:		break;
					case VDX_FOCUS_POS:		clr = 1;			break;
					case VDX_ZM_FCS_POS:	er = IF_NOEXCUTE;	break;
					case VDX_CAM_MEMCUS:
						if(tmp == 0x02)
							er = IF_NOEXCUTE;
			...
			case VDX_ZM_FCS_POS :
				switch(Vsc_socket[bfr].pCodec->eepidx)
				{
					case VDX_ZOOM_POS:		er = IF_NOEXCUTE;	break;
					case VDX_FOCUS_POS:		er = IF_NOEXCUTE;	break;
					case VDX_ZM_FCS_POS:	clr = 1;			break;
					case VDX_CAM_MEMCUS:
						if(tmp == 0x02)
							er = IF_NOEXCUTE;
						else
			...
			break;
			case VDX_CAM_MEMCUS :
				if(tmp == 0x00 || tmp == 0x01)//Reset , Set
				{
					switch(Vsc_socket[bfr].pCodec->eepidx)
					{
						case VDX_ZOOM_POS:		er = IF_NOEXCUTE;	break;
						case VDX_FOCUS_POS:		er = IF_NOEXCUTE;	break;
						case VDX_ZM_FCS_POS:	er = IF_NOEXCUTE;	break;
						case VDX_CAM_MEMCUS:	er = IF_NOEXCUTE;	break;
					}
				}
				else if(tmp == 0x02)
				{
			...
			break;
			default:
				if(Vsc_socket[bfr].pCodec->eepidx != VDX_CAM_MEMCUS)
				{
					er = IF_ERNO;
				}
				else if(tmp1 != 0x02)
	...
	else
	{
		...
		switch(vdx)
		{
			case VDX_ZM_FCS_POS:	er = IF_PASS;	break;
			case VDX_ZOOM_POS:		er = IF_PASS;	break;
			case VDX_FOCUS_POS:		er = IF_PASS;	break;
			case VDX_CAM_MEMCUS:	er = IF_PASS;	break;
		}
	}
	...
	if(!er || er == IF_PASS)
	{
		if(clr == 1)
		{
			// Ack
			gPtc_Visca_Socket = crr + 1;
			PtcVisca_SendPacket(VISCA_MSG_ACKNOWLEDGE,0,0);
			// Complete Before CMD
			gPtc_Visca_Socket = bfr + 1;
			Visca_Socket_Clear(bfr);
			printf("===========CMP2\n");
			PtcVisca_SendPacket(VISCA_MSG_COMPETE_CMD,0,0);
			gPtc_Visca_Socket = crr + 1;
		}
...
*************************************************************************************************************/
IFER Vsc_DecodeDat_CMDs(PTC_VISCA_CODEC *pCodec)
{
	IFER er = IF_ERNO;
	IFER evt_result = IF_ERNOPROC;
	...
	if(pCodec->evts)
	{
		...
		for(cnt = 0; plu_evt[cnt].eepidx != IDX_NULL ;cnt++)
		{
			Compose_data(plu_evt[cnt].type, plu_evt[cnt].length, &gPtcBuf.data[acc], &data[cnt]);

			if(!Visca_Condition_Check(plu_evt[cnt].eepidx,data[cnt]))
			{
				er |= Processong_CMD_Value_Chk(plu_evt[cnt].decode_type ,plu_evt[cnt].eepidx ,data[cnt] ,(void *)plu_evt[cnt].evts);
				if(g_ViscaData.FLD.PWR_ON)	//PWR_ON : ON(0) ,OFF(1)
				{
					if(!er)
						er |= IF_NOEXCUTE;
				}
		...
		if(!er)
			er = Visca_Socket_Set(pCodec->eepidx,data[0]);

		if(!er)
		{
			er = IF_Ptc_Vsc_SetDatas(plu_evt,data);
		}
		else if(er == IF_PASS)
		{
			if(Vsc_socket[0].pCodec)
				...
				tmp = (gPtc_Visca_Socket - 1)&0x01;

			Vsc_socket[tmp].pCodec = pCodec;
			Vsc_socket[tmp].execute = gExecute;

			for(cnt = 0; plu_evt[cnt].eepidx != IDX_NULL ;cnt++)
			{
				Vsc_socket[tmp].dat[cnt] = data[cnt];
			}

			if(Vsc_socket[tmp].execute)
			{
				er = IF_Ptc_Vsc_SetDatas(plu_evt,data);
				printf("????\n");
			}
...
*************************************************************************************************************/
IFER Vsc_DecodeDat_CMD(PTC_VISCA_CODEC *pCodec)
{
	IFER er = IF_ERNO;
	...
	if(!er)
		er = Visca_Socket_Set(pCodec->eepidx,data);

	if(!er)
	{
		er = IF_Ptc_Vsc_SetData(pCodec,data);
	}
	else if(er == IF_PASS)
	{
		if(Vsc_socket[0].pCodec)
...
IFER PtcViscaCMDProc()
{
	...
	{
		er = Vsc_DecodeDat_CMDs((PTC_VISCA_CODEC *)&gPtcVisca_Cmd_ZmFcs);
	}
	else if(pCodec->decode_type == DECODE_PLURAL)
	{	//복수의 변수 처리
		er = Vsc_DecodeDat_CMDs((PTC_VISCA_CODEC *)&gPtcVisca_Cmd[gDecode]);
	}
	else
	{
		er = Vsc_DecodeDat_CMD((PTC_VISCA_CODEC *)&gPtcVisca_Cmd[gDecode]);
	}

#ifdef WIN32
	printf("Curr Socket : (%d)\n",gPtc_Visca_Socket);
	if(Vsc_socket[0].pCodec)
	{
		printf("Socket 0 (%02X %08X %02X) ",Vsc_socket[0].pCodec->cmd,Vsc_socket[0].dat[0],Vsc_socket[0].execute);
		id_Print(Vsc_socket[0].pCodec->eepidx);
		printf("\n");
	}
	else
	{
		printf("Socket 0 (== ======== ==) EMPTY");
		printf("\n");
	}
	if(Vsc_socket[1].pCodec)
	{
		printf("Socket 1 (%02X %08X %02X) ",Vsc_socket[1].pCodec->cmd,Vsc_socket[1].dat[0],Vsc_socket[1].execute);
		id_Print(Vsc_socket[1].pCodec->eepidx);
		printf("\n");
	}
	else
	{
		printf("Socket 1 (== ======== ==) EMPTY");
		printf("\n");
	}
#endif
	return er;
...
IFER Vsc_Decode_VerINQ_Size()
{
	...
	#elif (VISCA_COMPILE == VSC_TYPE_EV7100)
		data = 0x0468;
	#elif (VISCA_COMPILE == VSC_TYPE_EX480)
		data = 0x0468;
	#else
		data = 0x045F;
	#endif
	...
	while(hd_size != -1 && end_size != -1 && flg) // 1vd에 1패킷을 처리하지만 반송되는 명령어는 여러 패킷을 처리한다.
	{
		...
		if(gPtcBuf.data[hd_size]&0x80)
		{
			...
			if(gPtc_Visca_Addr == stx)	//어드레스 일치
			{
				...
				else if((er & 0x10) == IF_SOCKET_FULL)
					PtcVisca_SendPacket(VISCA_ERROR_NO_SOCKET,0,0);
				/***********************************Packet Claer********************************/
				if(shft)
...
*	return 0 (완료)
* @brief
*	이 함수는 시간이 오래걸리는 명령이 처리되었는지를 리턴하는 함수이다.\n
*	따라서 이 함수를 CallBeck으로 등록하면 항상 0(완료)가 나온다.\n
*	Null 함수 : 함수를 초기화 하는 데 사용한다. \n
*	CallBeck 함수를 등록하지 않으면 이 함수를 Call한다. \n
*************************************************************************************************************/
uint08 Cam_Link_Null(void)
{
	return 0;
...
IFER IF_Send_Reply_Packet(VISCA_REPLY_TYPE reply ,uint32 dat)
{
	...
	{
		case VSC_REPLY_MD :
			er = IF_Send_Reply_MD((uint08)dat);
		break;
		case VSC_REPLY_ICR :
			er = IF_Send_Reply_ICR((uint08)dat);
		break;
		case VSC_REPLY_ZOOM :
			IF_Send_Reply_Zoom(dat&0xffff,dat>>16);
		break;
		case VSC_REPLY_FOCUS :
			IF_Send_Reply_Focus((uint08)dat);
		break;
		case VSC_REPLY_WD :
			IF_Send_Reply_WD((uint08)dat);
		break;
		default:
			er = IF_INVALIDVAL;
		break;
...
{
	volatile uint16 bf_head = Ptc_Que.TxHead++;
...
{
	volatile uint16 bf_head = Ptc_Que.RxHead++;
...
*	return IFER
*************************************************************************************************************/
IFER IF_VISCA_Regist_CamState(VISCA_CAM_STATE type,uint08 (*getstt)(void))
{
	IFER er = IF_ERNO;

	if(!getstt)
	{
		getstt = Cam_Link_Null;
		er = IF_ERNO;
	}

	switch(type)
	{
		case VISCA_STT_ZOOM_DIRCT :		Cam_Link_Zoom_Drt = getstt;		break;
		case VISCA_STT_FOCUS_DIRCT :	Cam_Link_Focus_Drt = getstt;	break;
		case VISCA_STT_ZMFCS_DIRCT :	Cam_Link_ZmFcs_Drt = getstt;	break;
		case VISCA_STT_RESET :			Cam_Link_Reset = getstt;		break;
		case VISCA_STT_SET :			Cam_Link_set = getstt;			break;
		case VISCA_STT_RECALL :			Cam_Link_Recall = getstt;		break;
